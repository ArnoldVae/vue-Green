<template style="background: #f1f1f1">
    <div>
        ...
        <el-dialog title="案件详情" :visible.sync="caseSupplementFlag" class="mc-dialog-big" width="100%" top="0">
            <CaseSupplement ref="caseReg" :myData="source" pageFlag="details"></CaseSupplement>
        </el-dialog>
        ...
        <el-dialog class="mc-dialog-big "  title="案件受理" :visible.sync="caseAcceptWindow" width="100%" top="0" style="background: #ffffff">
            <CaseAcceptWindow @submitEvent = closeFtn></CaseAcceptWindow>
        </el-dialog>
    </div>
</template>
...
<script lang="ts">
    ...
    import CaseAcceptWindow from "./modal/caseAcceptWindow.vue";

    @Component({
        name: "formAudit",
        source:{},
        components: {CaseSupplement, StateLine,CaseAcceptWindow},
        data() {
            return {
                form: {}
            };
