...
<!-- 会议审议弹窗-审理案件 -->
  	<div>
           <el-collapse v-model="activeNames" @change="handleChange" class="collapse" ref="csElCoolapase">
               <el-collapse-item  id="menu1"  name="1" >
                   <template slot="title">
                       <i class="header-icon el-icon-info"></i>本次会议审议信息
                       <el-button size="mini" type="primary" plain @click="viewResultsPage('viewResultsPage')" >查看历次审议记录</el-button>
                   </template>
      ...
      <el-dialog class="mc-dialog-medium" title="历次审议记录" :visible.sync="dialogFlag.viewResultsPage" width="80%"  top="13%" append-to-body center>
	      <ViewResultsPage></ViewResultsPage>
	      <div slot="footer" class="dialog-footer">
				<el-button size="small" @click="dialogFlag.viewResultsPage=false">关闭</el-button>
			</div>
    	</el-dialog>
	</div>
</template>
...
export default class caseSupplement extends Vue {
    ...
    showFlag:boolean = false;
      dialogFlag = {
            allPrevious:false,
            viewResultsPage:false,
        }
         viewResultsPage(item){
